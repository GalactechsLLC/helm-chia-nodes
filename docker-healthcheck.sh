...
nc -z -v -w5 localhost 55400
# shellcheck disable=SC2181
if [[ "$?" -ne 0 ]]; then
    aria2c_process_id=$(pidof aria2c)
    if [[ -z $aria2c_process_id ]]; then
        tar_process_id=$(pidof tar)
        if [[ -z $tar_process_id ]]; then
            # Wait and do one more check to prevent too many restarts
            sleep 5
            nc -z -v -w5 localhost 55400
            if [[ "$?" -ne 0 ]]; then
                aria2c_process_id=$(pidof aria2c)
    ...
    if [[ "$?" -ne 0 ]]; then
        ...
        if [[ -z $aria2c_process_id ]]; then
            ...
            if [[ -z $tar_process_id ]]; then
                ...
                -d '{}' -k -H "Content-Type: application/json" https://localhost:8555/healthz
                if [[ "$?" -ne 0 ]]; then
                    logger "$(dt) Fullnode healthcheck failed"
                fi
            fi
        fi
    fi
